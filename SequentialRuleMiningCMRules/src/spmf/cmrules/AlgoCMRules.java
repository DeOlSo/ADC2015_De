...
//	 a triangular matrix for efficiently counting the support of pairs of items
	 ...
	 */
	public ArrayList<String[]>  runAlgorithm(final BufferedDataTable[] input,String colName, double absoluteMinSupport, double minConfidence) throws IOException {
		// load the sequence database from the input file
		sequences = new SequenceDatabase();
		sequences.loadFile(input,colName);
		...
		runAlgorithm(minCSupRelative, minConfidence, input, colName);
	 ...
	 * @throws IOException exception if error while writing the output file.
	 */
	public void runAlgorithm(int relativeSupport, double minConfidence, final BufferedDataTable[] input, String colName) throws IOException {
...
//		
    ...
    public String getStatistics(){
		...
		r.append("=============  SEQUENTIAL RULES - STATS =============\n");
		r.append("Association rules count: " + associationRulesCount+"\n");
		r.append("Sequential rules count: " + ruleCount+"\n");
		r.append("Total time : " + (timeEnd - timeStart) + " ms\n");
		r.append("Max memory: " + MemoryLogger.getInstance().getMaxMemory()+"\n");
		r.append("===================================================");
        return r.toString();
    }
...
//				System.out.println(); // DEBUG
	...
	private void saveRule(Rule rule) throws IOException {
		...
		for(int i=0; i<rule.getItemset1().size(); i++){
			ruleString.append(rule.getItemset1().get(i));
		...
		for(int i=0; i<rule.getItemset2().size(); i++){
			ruleString.append(rule.getItemset2().get(i));
		...
		ruleList.add(new String[]{ruleString.toString(),""+rule.getSequentialAbsoluteSeqSupport(),""+rule.getSequentialConfidence()});
	}
